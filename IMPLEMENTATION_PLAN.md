...
## Phase 10 — Multi-vertical support (restaurant / appointment) ✅ done

Built from `MULTI_VERTICAL_PLAN.md` (phases M1–M5 there), which itself opens with the single most consequential finding of this phase: appointment booking was **not** a green-field build. Commits shipped between Phase 9 and this one (`#16`–`#19` — appointment scheduling/availability/reminders, the `INTERACTION_MODE` WhatsApp-Flow/browser-link controller, FAQ, and Meta Embedded Signup) had already built a full appointment-booking feature — domain, state machine, adapters, API, a native WhatsApp Flow, availability slots, reminders, and dashboard UI — but as an **additive opt-in toggle** (`Merchant.appointment_booking_enabled`) layered *alongside* restaurant ordering, not as an exclusive vertical. None of that intermediate work was written up as its own numbered phase here; this entry is the first to fold it into the plan's narrative, as the thing Phase 10 converts rather than builds from scratch.

**Deviations from `MULTI_VERTICAL_PLAN.md`, and why:**
- **None of substance.** The plan's five phases (M1 migration/state-machine, M2 onboarding wizard, M3 nav/Services page, M4 WhatsApp menu, M5 cleanup) were built in order, each gated on the previous one's tests passing, exactly as scoped. The one refinement made during implementation: M1's `try_advance_for_catalog_ready` cascade for the appointment vertical is routed *through* the `catalog_ready` status (an always-satisfied gate) rather than a direct `profile_completed → live` edge, preserving `ONBOARDING_TRANSITIONS`' "strictly linear, no step-skipping" invariant instead of adding a new transition-table edge — the plan document described the *effect* ("cascades directly") without committing to that specific mechanism.

**Backend, as built:**
- `identity/domain/models.py`: `MerchantVertical` enum (`restaurant`/`appointment`); `Merchant.vertical`, nullable until set. New `vertical_selected` onboarding status between `registered` and `meta_connected`. `MerchantRepository.set_vertical` raises `VerticalAlreadySetError` on a second call — structural enforcement of "no admin path to change a merchant's vertical," not just an absent UI affordance.
- `onboarding/domain/onboarding_service.py`: `advance_after_vertical_selected` (same idempotency convention as the existing `advance_after_*` functions); `try_advance_for_catalog_ready` is vertical-aware — the restaurant item-gate is unchanged, the appointment vertical's gate is unconditionally satisfied (services are optional by design, matching `AppointmentService`'s own existing docstring).
- `PUT /api/v1/onboarding/vertical`: one-time, 409 on repeat, 422 on an unrecognized value.
- `conversation/domain/intents.py` / `handler.py`: `Intent.TALK_TO_RESTAURANT` removed outright (both verticals, unconditionally); new `Intent.TRACK_APPOINTMENT`, keyword-ordered ahead of `TRACK_ORDER`/`BOOK_APPOINTMENT`'s broader terms. The greeting menu and every intent branch became exclusive on `Merchant.vertical` instead of additive on the old toggle — a restaurant merchant's menu is exactly Place order / Track order (+ FAQs), an appointment merchant's is exactly Book appointment / Appointment status (+ FAQs), never both, never the removed handoff option. `TRACK_APPOINTMENT` reuses `AppointmentRepository.list`'s existing `customer_id` filter (already soonest-upcoming-first) rather than adding a new repository method.
- `appointment_flow/api/router.py`'s public booking-webview gate and `shared/scheduler.py`'s reminder-scan merchant list both moved off `appointment_booking_enabled` onto `Merchant.vertical` in the same phase that removed the column (Phase M5), so there was never a window where a code path read a dropped column.
- Migration (Phase M1): backfills every pre-existing merchant to `restaurant`, except one already opted into the additive toggle, backfilled to `appointment` — preserving current behavior for both groups. Migration (Phase M5): drops `appointment_booking_enabled` once its last reads (above) were gone.
- Tests: 6 new onboarding-flow tests (vertical selection, immutability, unknown-value rejection, the appointment vertical's gate-free `live` cascade); intent-classification tests for `TRACK_APPOINTMENT` including its ordering-sensitivity case; conversation-handler tests asserting exclusivity in both directions (a restaurant merchant never gets an appointment reply and vice versa) plus the new track-appointment branch; `test_appointment_flow.py`/`test_scheduler.py`/`test_identity.py` updated off the retired toggle.

**Frontend, as built:**
- `features/onboarding/OnboardingPage.tsx`: new `VerticalSelectStep` (wizard step 0) — two choice cards on first visit, then read-only once answered (mirrors the backend's immutability rather than offering an edit path the API would just 409). `BusinessDetailsStep` branches its address/category/license copy by vertical (the underlying columns were already vertical-neutral, per Phase 8's earlier rename). New `AddServiceStep` (restaurant's `AddItemStep` analogue), reusing the appointment-services hooks that already existed rather than duplicating them.
- `shared/components/Layout.tsx`: nav is a function of `Merchant.vertical` — Orders+Catalog for restaurant, Appointments+Services for appointment, neither pair before a vertical is chosen.
- New `features/services/ServicesPage.tsx` + `/services` route: the service-management UI that had been a Settings-page section (`AppointmentServicesSettingsSection`) got its own page and nav tab, mirroring how Catalog already isn't buried in Settings — same underlying hooks, relocated call site, not rebuilt.
- Settings page: WhatsApp Flow setup cards (Order Flow / Appointment Flow) gated by vertical instead of the toggle; the toggle's own settings section and hook file (`useAppointmentSettings.ts`) removed entirely in Phase M5, along with the now-appointment-only availability-hours section (gated by vertical rather than shown to every merchant).
- Tests: `OnboardingPage.test.tsx` extended for the new step and both verticals' branching; new `Layout.test.tsx` (nav exclusivity in both directions, plus the "neither before a vertical is chosen" case) and `ServicesPage.test.tsx` (list/empty-state/create, mirroring `CatalogPage.test.tsx`'s pattern).

**Docs updated to match:** `ARCHITECTURE.md` §1 (`Merchant.vertical`), §5 (onboarding status table, including the appointment vertical's gate-free `catalog_ready`), §6 (intent-menu framing), §9b/§9c diagrams (vertical-choice wizard step; two branches from the intent-menu node, "Talk to us" removed from both), §11 (the appointment vertical realizes the brief's own "vertical-agnostic by design" statement, not a deviation from it).

**Definition of done** (met): two merchants — one `restaurant`, one `appointment` — each complete their own onboarding wizard from a clean register (the vertical step visibly branches everything after it), land on a dashboard whose nav shows exactly one of Orders/Appointments and one of Catalog/Services, and each merchant's simulated WhatsApp greeting offers only the options valid for their vertical with "Talk to us" gone from both. `appointment_booking_enabled` no longer exists anywhere in the codebase (schema, backend, or frontend). Backend: 579 tests passing, `ruff`/`mypy` clean (one pre-existing, unrelated `mypy` finding in `payments/api/router.py` predates this phase). Frontend: 140 tests passing, `biome`/`tsc` clean.

---

## Phase 11 — Vertical toggle: multi-select onboarding + Settings add-on ✅ done

Built from `VERTICAL_TOGGLE_PLAN.md`, written before any code per the same convention `MULTI_VERTICAL_PLAN.md` established for Phase 10. Converts Phase 10's "exactly one vertical, chosen once, immutable" model into an additive one: a merchant can run restaurant ordering, appointment booking, or both, selected together at onboarding or added later from Settings — no re-registration, no admin intervention, no separate code path.

**Deviations from `VERTICAL_TOGGLE_PLAN.md`, and why:**
- **None of substance.** The plan's six phases (T1 data model, T2 onboarding gate, T3 API surface, T4 WhatsApp additive menu, T5 frontend, T6 docs) were built in order, exactly as scoped, each verified against the full test suite before moving on.

**Backend, as built:**
- `identity/domain/models.py`: `Merchant.vertical` (`str | None`) replaced by `Merchant.restaurant_enabled`/`appointment_enabled` (both `bool`, default `False`). New `validate_vertical_flags`/`NoVerticalSelectedError` — the one place the "not both False" invariant is checked, called from `MerchantRepository.set_vertical_flags`, the only writer for both flags. No one-time/immutability guard any more (Phase 10's `VerticalAlreadySetError` is gone) — `set_vertical_flags` is called both by the onboarding wizard's first step and, later, by Settings, with identical validation either way.
- Migration: adds both boolean columns (`server_default=false` to satisfy `NOT NULL` against existing rows), backfills each merchant from their old `vertical` value (`COALESCE(vertical = 'restaurant', false)` / same for appointment — a still-mid-onboarding merchant with `vertical IS NULL` backfills to both `False`, the same transient pre-selection state the nullable column used to represent), then drops `vertical`. Downgrade collapses a hypothetical both-`True` row to `'restaurant'` (the old column can't represent "both").
- `onboarding/domain/onboarding_service.py`: `try_advance_for_catalog_ready`'s gate becomes "every *enabled* vertical is ready" (`restaurant_ready`/`appointment_ready`, new exported helpers — a disabled vertical trivially satisfies its own half). This **overrides MULTI_VERTICAL_PLAN.md's Decision 5**: the appointment vertical's "services are optional, no gate" rule is retired — under the additive model, an enabled-but-empty appointment vertical is exactly the "empty flow" a customer must never be shown, so it's now symmetric with restaurant's `≥1 available Item` gate. `identity/api/router.py`'s appointment-service create/update endpoints call the gate now too (mirroring `catalog/api/router.py`'s item endpoints), since appointment readiness became a real gate for the first time.
- `PUT /api/v1/onboarding/vertical` (single value) → `PUT /api/v1/onboarding/verticals` (`{restaurant_enabled, appointment_enabled}`, multi-select, 422 on both-false, no 409 ever) — literally the same endpoint the onboarding wizard and Settings' new "Business types" section both call.
- `conversation/domain/handler.py`: the WhatsApp menu/reply-branch gating changed from exclusive-on-`vertical` to additive-on-`(enabled AND ready)` per vertical — a both-enabled-and-ready merchant's greeting offers all four options (tipping over WhatsApp's 3-button cap into a list message); an enabled-but-not-ready vertical is silently omitted, the same empty-flow guard the onboarding gate enforces, checked fresh on every inbound message via the same `restaurant_ready`/`appointment_ready` helpers (no duplicated logic, no separate "is this vertical live yet" flag to keep in sync). `appointment_flow/api/router.py`'s public booking-webview 404 gate and `shared/scheduler.py`'s reminder-scan merchant list moved off the old `vertical` column onto `appointment_enabled` in the same phase, so there was no window reading a dropped column.
- Tests: onboarding-flow invariant tests (both-false rejected with 422 regardless of caller), both-enabled-requires-both-gates cascade test, appointment-now-requires-a-service test (regression check against the old "optional" behavior), "callable again later, no 409" test standing in for the Settings add-on path; conversation-handler tests for the additive menu (all-four-options case, enabled-but-not-ready-is-omitted case) plus every pre-existing per-vertical test updated to seed the new readiness data its gate now requires.

**Frontend, as built:**
- `shared/api/types.ts`/`Layout.tsx`: `Merchant.vertical` → `restaurant_enabled`/`appointment_enabled`; nav becomes additive (`navItemsForVerticals`) instead of an either/or lookup — both vertical tab-pairs show at once when both are enabled.
- `features/onboarding/OnboardingPage.tsx`: `VerticalSelectStep` rewritten as multi-select checkboxes (was two mutually-exclusive cards) and, since the backend dropped the immutability guard, stopped being the one permanently-read-only step — it's editable like every other step now, consistent with the data actually being editable. The wizard's step sequence became dynamic: one setup step per selected vertical (item step, then service step, restaurant-then-appointment always — the plan's explicit "simpler and more robust than tracking click order" decision, not "whichever was checked first"), computed from `OnboardingStatusOut`'s existing `has_available_item`/`has_available_service` fields rather than a new per-vertical status.
- `features/settings/SettingsPage.tsx`: new "Business types" section — two switches bound to the same flags, saving through the identical `PUT /verticals` mutation the wizard uses. The existing vertical-gated Settings sections (WhatsApp Flow setup cards, appointment availability hours) switched from `vertical === 'x'` to `x_enabled`, so both can render at once.
- Tests: `OnboardingPage.test.tsx` extended for multi-select (single pick, both-at-once, Continue disabled until ≥1 picked, previously-selected flags shown pre-checked and still editable, both-enabled sequential item-then-service step ordering); `Layout.test.tsx` gained the both-enabled-shows-all-four case; `SettingsPage.test.tsx` gained a new describe block for the Business types section (pre-checked from `Merchant`, blocks saving at both-off, submits the same payload the wizard would).

**Docs updated to match:** `ARCHITECTURE.md` §1 (`restaurant_enabled`/`appointment_enabled`, invariant, add-on-later path), §5 (onboarding status table's `catalog_ready` gate description, the "add a vertical after live" no-status-change note), §6 (additive intent menu framing, the empty-flow guard), §9b/§9c diagrams (multi-select wizard step branching into per-vertical setup steps; intent-menu decision framed as additive, not `Merchant.vertical`-exclusive), §11 (deviation #5 updated off the old immutable-enum framing).

**Definition of done** (met): registering a fresh merchant and picking both business types on the wizard's first step shows the item-setup step, then (once an item exists) the service-setup step, before FAQs/Go-live; the merchant lands `live` with a dashboard nav showing Orders, Catalog, Appointments, and Services all at once, and a simulated WhatsApp greeting offering all four options. A second, restaurant-only merchant's WhatsApp greeting shows only Place order/Track order until they add appointment booking from Settings and add a service — at which point, with no onboarding re-entry, the greeting picks up Book appointment/Appointment status too. Backend: 584 tests passing, `ruff`/`mypy` clean (same one pre-existing `payments/api/router.py` finding, unrelated to this phase). Frontend: 148 tests passing, `biome check`/`tsc -b`/`vite build` all clean.
