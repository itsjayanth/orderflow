...
import { useSetupWhatsAppAppointmentFlow } from './useWhatsAppSettings'
...
export function AppointmentFlowSetupCard({ disabled = false }: { disabled?: boolean }) {
  const [wabaId, setWabaId] = useState('')
  const setupFlow = useSetupWhatsAppAppointmentFlow()
  ...
  return (
    <div className="max-w-md space-y-3 border-t pt-4">
      <div>
        <h3 className="text-sm font-medium">Enable native appointment booking</h3>
        <p className="text-muted-foreground text-xs">
          Let customers book a slot without ever leaving WhatsApp.
        </p>
      </div>

      {setupFlow.isSuccess ? (
        <SavedIndicator message="Native appointment booking enabled!" />
      ) : (
        <div className="space-y-2">
          <Label htmlFor="appointment_meta_waba_id">WhatsApp Business Account ID</Label>
          <div className="flex items-center gap-3">
            <Input
              id="appointment_meta_waba_id"
      ...
      {disabled && (
        <p className="text-muted-foreground text-xs">
          Save your credentials above before enabling native appointment booking.
        </p>
      )}
      {setupFlow.isError && (
