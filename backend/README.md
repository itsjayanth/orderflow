...
**Connecting WhatsApp has two independent paths onto the same `WhatsAppBusinessAccount` row**, both ending at the same `advance_after_whatsapp_connected()` call: the manual paste flow above (`PUT /api/v1/onboarding/whatsapp`), and Meta's WhatsApp Embedded Signup (`onboarding/domain/embedded_signup.py`, `POST /api/v1/onboarding/whatsapp/embedded-signup/complete`) — the frontend runs Meta's Facebook Login for Business JS SDK popup, then hands the resulting `code`/`waba_id`/`phone_number_id` to the backend, which exchanges the code for a long-lived token, registers the phone number for API messaging (`POST /{phone_number_id}/register`, required once per number before it can send/receive at all), and subscribes the app to the WABA's webhooks (`POST /{waba_id}/subscribed_apps`). Everything downstream (`conversation/adapters/whatsapp_client.py`, `notifications/`, `flows/`) reads `phone_number_id`/`access_token_encrypted` off the row regardless of which path set them, so nothing else needed to change. Embedded Signup needs app-level Meta credentials (`META_APP_ID`/`META_APP_SECRET`/`META_CONFIGURATION_ID`/`META_GRAPH_API_VERSION` — see `.env.example`); `GET /api/v1/onboarding/whatsapp/embedded-signup/config` reports whether they're set so the frontend can hide the "Connect WhatsApp" button rather than launch a popup that can only fail. Without them, the manual flow works exactly as before.
...
| `META_APP_ID`, `META_APP_SECRET`, `META_CONFIGURATION_ID` | from the Meta App Dashboard — only needed to enable the "Connect WhatsApp" Embedded Signup button; the manual credential-paste flow works without these |
| `META_GRAPH_API_VERSION` | e.g. `v21.0` — defaults to `v21.0` if unset |
